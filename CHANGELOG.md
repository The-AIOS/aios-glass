...
- **AIOS Files** — a tidy **collapsible file tree** (its own activity-bar container, so Home reads cleanly as just *AIOS Glass*); open it from the explorer button in the Home header, `AIOS: Browse Files`, or `⌘⌥G B`. Three labelled sections differentiate the substrate at a glance — **FRAMEWORK** (AIOS infra: agents/skills/plugins/commands + the root docs; dimmer ring dot), **VAULT** (your notes — the primary surface, solid coral dot), **WORKSPACE** (external folders you add: repos, drives — `+` to add, each removable, persisted in `.glass/state.json`). Click a folder to expand inline, a file to open it (same viewer logic as Home — `.md` → preview, `.html` → browser, etc.). Right-click any row → *Reveal in Finder*. Numbered-folder prefixes are dimmed, build/noise dirs hidden, and path access is sandboxed to the section roots. (The explorer follows the global theme — no toggle of its own.)
