...
### Fixed
- **A release for a sibling looped back to the releaser.** Captured from the durable bus log, three lines 100ms apart: `released … for a sibling; release 1/2` · `release 2/2` · `DEAD LETTER — never delivered`. A surface claimed a send for a session hosted in the *other* surface, correctly found it had no terminal for it, handed the request back — and **re-claimed its own release**, twice, spending `MAX_RELEASES` before the sibling's watcher fired. The bias is structural rather than unlucky: a release writes the file back into the directory the releaser is already watching with a hot watcher, while a sibling waits on an OS notification. `MAX_RELEASES` bounded *bounces*, not *fulfillers*, so the whole budget could go on a self-conversation. A release now records `_tried`, and a fulfiller refuses to re-claim what it already tried.
- **`_tried` is keyed by INSTANCE (`surface:pid`), not by surface.** This file's own release doc says *"another window may own that terminal"* and `MAX_RELEASES` is documented as *"sibling-window handoffs"* — two Glass windows are two candidate fulfillers of one surface, so keying by name would have broken the very handoff the bound exists for. Caught when a leftover dev instance raced a fresh one and both were `glass`.
- **Nothing rots.** With the releaser abstaining, a request no sibling takes would have sat forever. The releaser — already awake — now schedules the honest ending: re-check after the retire grace, then dead-letter. Without it, `_tried` would have traded a false dead letter for a **silent** one.
- **The surface presence record named a pid no session descends from.** Shipped hours earlier on the stated belief that *"a terminal Glass creates descends from the extension host"*. Measured: the extension host was a **Helper (Plugin)** process while a terminal it owns descended from the **ptyHost**, a plain Helper — *siblings*, both children of the IDE's Electron root. The derivation returned "neither" for every IDE-hosted session, so the feature would have shipped doing nothing at all, silently. No unit test could have caught it: the logic was right and the model of the process tree was wrong. Glass now announces the **process-tree root**, bounded at 32 hops and cycle-safe, because a hang at activation is worse than a wrong answer.

### Added
- **Surface presence, so a requester can spawn where it already lives.** An unaddressed `spawn` goes to whichever surface wins the race, and a spawned session **stays** wherever it landed — unlike a `send`, there is nothing to hand back. Measured: a request written from an IDE session was fulfilled by the App, and the worker opened as a pane instead of the IDE terminal the requester meant. Glass announces its root pid at `~/.aios/surfaces/glass.json` — **outside** the inbox, because the watchers claim `*.json` and a presence file there would be read as a request. Requesters compare **pids, never process names**: Glass runs inside whatever IDE the operator uses, so a name list breaks the day they switch editors.
- **The inbox README documents the derivation**, with the snippet run verbatim before being written down, and states plainly that **no match is a legitimate answer** — a `spawn`-wrapper session in a plain terminal belongs to neither surface and must stay reachable by either.

### Verified
Two surfaces on one isolated inbox, in an extension development host: a **glass-addressed** send delivered in under 4s (target replied) · an **unaddressed** send for a Glass-hosted session — the App released **once**, abstained, and **Glass delivered**, under 4s total, where the same request that morning died in 200ms without Glass ever seeing it · the README's derivation snippet, run inside the target session, printed `glass`, matched 4 hops up at the announced root. 78 unit tests, and every new guard confirmed to go red when its fix is removed.

## [0.5.3] — 2026-08-14

> **The bug this release was scoped to port turned out to be here already — and my note said it wasn't.** `AI-93` was written as "port two changes; Glass is already correct on the baseline." Two of its three claims were wrong, and the third was the dangerous one.

### Fixed
- **The verification baseline moved.** `let baseline = 0` sat **inside** `for (;;)`, so it was re-read at the top of every iteration: if attempt 1 landed late, iteration 2 read a baseline that already contained that arrival, `now − before` was 0, the verdict stayed `pending` forever, and the message was re-typed every cycle for the whole hold budget. This is the other half of the App's four-deliveries bug — **the half that made it unrecoverable** — and it was live here the entire time. The scoping note said not to port the fix because Glass was already correct; following it would have shipped a "parity" release still carrying the primary defect.
- **A slash-command needle identified the COMMAND, not the DELIVERY.** `safeNeedle` returns the first run of ≥24 safe characters, or the first 24 when there is none — and a slash command is shorter than that, so `safeNeedle('/config')` is `'/config'`. Measured across twelve real transcripts (11,152 user records): **7 of 10** slash-command sends had a needle also present in other user records, invocations forty minutes and five hours apart. Combined with a fail-soft baseline (`catch { baseline = 0 }`), transcript **history** could read as a fresh arrival — the bus concluding its send landed, deleting the request, and telling nobody. A silently lost message, strictly worse than a duplicated one. Counting is now bounded to records at or after the claim, and a record without a parseable timestamp is **not counted** — the same asymmetry as the deliverability allowlist, since counting it risks a silent loss while skipping it risks only a retry.
- **A slash command was typed three times.** `/help` was delivered — the operator watched the menu appear — while the transcript recorded **zero** user turns containing the marker, because the CLI handles it client-side and writes nothing. Verification failed for a delivery that had plainly succeeded and the cap was the only thing that stopped it. Retry only pays when a miss is *evidence* of a failed send; for a command the CLI handles locally it is not, and the class is not uniform (`/config` **does** write a record, `/help` does not, and nothing outside the CLI can tell which). A slash command is now typed **once** and then waited on for the full hold budget — waiting still catches a command that verifies late, re-typing catches neither and shows the operator a duplicate.

### Changed
- **`decideAfterVerifyMiss` gained `targetBusy` and lost `release`.** A target that went busy during verification has not had the *opportunity* to write the turn, so silence proves nothing; treating it as failure is what produced four deliveries of one request. And `release` was **removed from the type** rather than left unreachable — a type saying a decision can hand an *already-typed* message to a sibling invites a caller to handle that case, and handling it is the double-delivery bug. The sibling handoff still exists, bounded, on the path where nothing was typed.
- **The cross-repo contract now guards BEHAVIOUR, not just values.** `TIMINGS` is hashed because it is a data literal; the functions around it cannot be, because the two surfaces' lint regimes disagree about braces on single-statement bodies — so hashing their source would fail forever and teach everyone to ignore the one test guarding cross-process behaviour. A shared **decision table** is byte-identical across both repos and hashed instead.

### Added
- **Dev-host isolation, by default.** The inbox is machine-global, so an Extension Development Host launched for testing became a **third fulfiller** competing with the operator's installed Glass and installed App for real requests — which is how a live brief died on 2026-07-27. `launch.json` now points a dev host at a scratch inbox, so every run is isolated including the ones started from the Run menu by someone who never read this file. `AIOS_BUS_DISABLED=1` also exists; `AIOS_BUS_DIR` is honoured only outside `ExtensionMode.Production`, so a shipped build can never have its inbox redirected by a stray environment variable.
